...
Name: wandbox-boost-1.47.0-clang-3.2
Version: 1.47.0
Release: 1
License: Boost
Group: wandbox
...
%define _prefix /opt/wandbox/boost-1.47.0-clang-3.2
...
%build
find . -maxdepth 1 -name 'boost-1.47.0-clang-3.2*.patch' -print0 | xargs -0 cat | patch -p1
...
LD_LIBRARY_PATH=%{_clangdir}/lib:%{_clangdir}/lib64:%{_clanggccdir}/lib:%{_clanggccdir}/lib64 ./b2 toolset=clang stage release link=shared runtime-link=shared --without-mpi --without-signals --without-python --without-wave %{_smp_mflags}

%install
LD_LIBRARY_PATH=%{_clangdir}/lib:%{_clangdir}/lib64:%{_clanggccdir}/lib:%{_clanggccdir}/lib64 ./b2 toolset=clang install release link=shared runtime-link=shared --without-mpi --without-signals --without-python --without-wave --prefix=%{buildroot}%{_prefix}
