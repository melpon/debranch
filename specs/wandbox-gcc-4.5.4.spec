...
cd gcc-%{version}
find .. -maxdepth 1 -name "gcc-4.5.4*.patch" -print0 | xargs -0 cat | patch -p1
cd ..
...
%{configure} --enable-languages=c,c++   --enable-checking=release --disable-nls --disable-multilib LDFLAGS="-Wl,-rpath,%{_prefix}/lib,-rpath,%{_prefix}/lib64,-rpath,%{_prefix}/lib32"
%{__make} %{_smp_mflags}
