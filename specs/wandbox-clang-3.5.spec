...
Source0: llvm-3.5.tar.xz
Source1: clang-cfe-3.5.tar.xz
Source2: clang-rt-3.5.tar.xz
Source3: clang-tools-extra-3.5.tar.xz
Source4: libcpp-3.5.tar.xz
...
%define _prefix /opt/wandbox/llvm-3.5
...
mv llvm-3.5 source
mv clang-cfe-3.5 source/tools/clang
mv clang-rt-3.5 source/projects/compiler-rt
mv clang-tools-extra-3.5 source/tools/clang/tools/extra
mv libcpp-3.5 libcxx
cp %{SOURCE10} .
for f in %{SOURCE11} %{SOURCE12} %{SOURCE13} %{SOURCE14}; do
  sed "s#@@prefix@@#%{_prefix}#" $f > $(basename $f .in)
done
cd source/tools/clang
find ../../.. -maxdepth 1 -name "clang-3.5*.patch" -print0 | xargs -0 cat | patch -p1
cd ../../..
cd libcxx
find .. -maxdepth 1 -name "libcxx-3.5*.patch" -print0 | xargs -0 cat | patch -p1
cd ..
true
