...
Name: wandbox-boost-1.54.0-clang-3.2
Version: 1.54.0
Release: 3
License: Boost
Group: wandbox
...
%define _prefix /opt/wandbox/boost-1.54.0-clang-3.2
...
%build
find . -maxdepth 1 -name "boost-1.54.0-clang-3.2*.patch" -print0 | xargs -0 cat | patch -p1
...
./b2 toolset=clang stage release link=shared runtime-link=shared --without-mpi --without-signals --without-python --without-test --without-log %{_smp_mflags}

%install
./b2 toolset=clang install release link=shared runtime-link=shared --without-mpi --without-signals --without-python --without-test --without-log --prefix=%{buildroot}%{_prefix}
