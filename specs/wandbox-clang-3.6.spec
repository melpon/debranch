...
Name: wandbox-clang-3.6
Version: 3.6
Release: 9
License: Modified BSD
Group: wandbox
BuildRoot: %{_tmppath}/%{name}-%{version}-%{release}-buildroot
Requires: wandbox-clang-3.4 g++
BuildRequires: wandbox-cmake
URL: http://melpon.org/wandbox
Source0: llvm-3.6.tar.xz
Source1: clang-cfe-3.6.tar.xz
Source2: clang-rt-3.6.tar.xz
Source3: clang-tools-extra-3.6.tar.xz
Source4: libcpp-3.6.tar.xz
...
%define _prefix /opt/wandbox/llvm-3.6
%define _ccprefix /opt/wandbox/llvm-3.4
...
mv llvm-3.6 source
mv clang-cfe-3.6 source/tools/clang
mv clang-rt-3.6 source/projects/compiler-rt
mv clang-tools-extra-3.6 source/tools/clang/tools/extra
mv libcpp-3.6 libcxx
cp %{SOURCE10} .
for f in %{SOURCE11} %{SOURCE12} %{SOURCE13} %{SOURCE14}; do
  sed "s#@@prefix@@#%{_prefix}#" $f > $(basename $f .in)
done
cd source/tools/clang
find ../../.. -maxdepth 1 -name "clang-3.6*.patch" -print0 | xargs -0 cat | patch -p1
cd ../../..
cd libcxx
find .. -maxdepth 1 -name "libcxx-3.6*.patch" -print0 | xargs -0 cat | patch -p1
cd ..
true
...
export CXX="%{_ccprefix}/bin/clang++ -stdlib=libc++ -I/opt/wandbox/llvm-3.4/include/c++/v1"
